...
namespace BlazorForms.Rendering
{
    public class ListFormViewModel : RenderingViewModel, IListFormViewModel
    {
        ...
        public async Task LoadModel(string flowType, string pK, FlowParamsGeneric flowParams, bool clear = false)
        {
            ...
            ApplyMaxLength(data.UserViewDetails);
        }

        private void ApplyMaxLength(FormDetails form)
        {
            for (int i = 0; i < form.Fields.Count; i++)
            {
                var maxLength = (form.Fields[i].DisplayProperties as FormDisplayDetailsExtended)?.MaxLength;
                ...
                if (maxLength != null)
                {
                    foreach (var line in ListData)
                    {
                        ...
                        {
                            line[i] = cell.Substring(0, maxLength.Value) + "...";
                        }
                    }
                }
            }
